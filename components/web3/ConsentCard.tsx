...
export function ConsentCard() {
  ...
  const handleGrantAccess = () => {
    ...
    // Simulate smart contract delay transaction
    setTimeout(() => {
      setAccess({
        doctorAddress: addressInput,
    ...
    <div className="flex flex-col bg-white border border-[#E5E5E5] rounded-lg p-5">
      <div className="flex items-center justify-between mb-5 pb-4 border-b border-[#F0F0F0]">
        <div>
          ...
          <p className="text-xs font-normal text-[#999] mt-1">Web3 Medical Consent Layer</p>
        </div>
        <WalletConnect />
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className="flex-1 flex flex-col gap-4">
          <div>
            <label className="block text-xs uppercase tracking-widest text-[#999] mb-1.5">Doctor Wallet Address</label>
            <input 
              type="text" 
              placeholder="0x..." 
              value={addressInput}
          ...
          <div>
            <label className="block text-xs uppercase tracking-widest text-[#999] mb-1.5">Expiry Date</label>
            <input 
              type="date" 
              value={expiryInput}
          ...
          {showSuccess && (
            <div className="text-xs font-medium text-emerald-500 mt-1">
              ✓ Access granted successfully on chain computation
            </div>
          )}
        </div>
        ...
        <div className="flex-1 flex flex-col border border-[#F0F0F0] rounded-lg p-4 bg-[#FAFAFA]">
          <h4 className="text-xs uppercase tracking-widest text-[#999] mb-3">Active Permissions</h4>
          ...
          {access ? (
            <div className="flex flex-col gap-3">
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs font-normal text-[#999]">Valid until {access.expiryDate}</span>
              </div>
            ...
            <div className="flex flex-col items-center justify-center flex-1 py-6">
              <span className="text-sm font-normal text-[#999]">No active access shared</span>
            </div>
          )}
        </div>
      </div>
